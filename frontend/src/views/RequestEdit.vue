<template>
  <div class="flex justify-center">
    <div class="mt-4 -mb-3 w-screen">
      <form action="" class="mx-2" method="get" v-on::submit="onSend">
        <div class="flex flex-col">
          <input v-model="form.title" class="bg-gray-100 mt-2 px-3 py-2 rounded-lg" type="text" name="title" placeholder="Заголовок">

          <select v-model="form.status" name="status" id="" class="mt-2 px-3 py-2 rounded-lg bg-gray-100 disabled:bg-gray-300" >
            <option value="1">Новая</option>
            <option value="2">В работе</option>
            <option value="3">Завершена</option>
            <option value="4">Отменена</option>
          </select>

          <div class="mt-2">
            <textarea v-model="form.text" placeholder="Текст заявки" class="bg-gray-100 px-3 py-2 w-full rounded-lg" type="text" name="text"></textarea>
          </div>

          <div class="mt-2">
            <label class="block mb-1 text-gray-900 dark:text-white" for="multiple_files">
              Изображения
            </label>
            <input v-on:change="onFileChange" class="block w-full py-1 px-1 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="multiple_files" type="file" multiple>
          </div>
        </div>
        <div class="flex w-full justify-end mt-5">
          <button class="bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded-lg">
            Отправить
          </button>
        </div>
      </form>
    </div>
  </div>
</template>

<script>
import axios from "../axios";
export default {
  name: "RequestEdit",
  data(){
    return {
      form : {
        title: '',
        status: '1',
        text: '',
        images: []
      }
    }
  },
  methods: {
    onSend: function (event) {
      event.preventDefault();
      let formData = new FormData();
      for(let input in this.form){
        formData.append(input, this.form[input]);
      }
      for( let i = 0; i < this.form.images.length; i++ ){
        let file = this.form.images[i];
        formData.append('image[' + i + ']', file);
      }
      console.log(formData);

      axios.post('/request', formData,
          {
            headers: {
              'Content-Type': 'multipart/form-data'
            }}
        )
        .then((response)=>{
          console.log(response);
        })
        .catch((response)=>{
          console.log(response);
        });
    },
    onFileChange: function (event) {
      this.form.images = event.target.files;
    },
  }
}
</script>

<style scoped>

</style>